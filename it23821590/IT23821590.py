...
def _extract_sinhala_only(page) -> str:
    """Extract only the Sinhala output text from the right panel."""
    try:
        result = page.evaluate("""() => {
            const el = document.querySelector('.w-full.h-full.p-3.pb-8.rounded-lg.ring-1.ring-slate-300.whitespace-pre-wrap.overflow-y-auto.bg-slate-50');
...
def _read_output(is_chat: bool, output_locator) -> str:
    if is_chat:
        # Try inner_text first (works for div output boxes)
        try:
            v = output_locator.inner_text()
            if v is not None:
                v = str(v).strip()
                # Filter out placeholder-like text
                if v and v not in ["0 characters", "Sinhala", "Transliterated Sinhala will appear here..."]:
                    # If it contains page nav text, it's the whole page - skip
                    if "PixelsSuite" not in v and "Privacy" not in v:
                        return v
        except Exception:
            pass
        try:
...
def _click_chat_tab(page):
    try:
        for pattern in [r"Chat Sinhala", r"Chat", r"Singlish"]:
            tab = page.get_by_role("tab", name=re.compile(pattern, re.IGNORECASE)).first
        ...
        for pattern in [r"Chat Sinhala", r"Chat", r"Singlish"]:
            ...
            if btn.count() > 0 and btn.is_visible():
                ...
                page.wait_for_timeout(1000)
                return
        for pattern in ["Chat Sinhala", "Chat", "Singlish"]:
        ...
        print(f"Note: Could not click Chat tab: {e}")
...
def _find_chat_locators(page, timeout_ms: int):
    ...
    while time.time() < deadline:
        ...
        try:
            # pixelssuite chat-translator: Singlish textarea + Sinhala output div
            input_by_ph = page.locator('textarea[placeholder*="Singlish"]').first
            # Output is a div inside the Sinhala panel (right side)
            output_div = page.locator('.output, [class*="output"], [class*="sinhala"], [class*="result"]').first
            if input_by_ph.count() > 0 and input_by_ph.is_visible():
                action = page.get_by_role("button", name=re.compile(r"(Transliterate|Translate)", re.IGNORECASE)).first
                # Use the right-side panel div as output
                right_panel = page.locator('div').filter(has_text=re.compile(r"0 characters|Sinhala")).nth(1)
                return input_by_ph, right_panel, action
        except Exception:
            pass
        ...
        try:
            ...
            if input_by_ph.count() > 0 and input_by_ph.is_visible():
                action = page.get_by_role("button", name=re.compile(r"(Transliterate|Translate)", re.IGNORECASE)).first
                # fallback: grab all textareas, first is input
                all_ta = page.locator("textarea")
                if all_ta.count() >= 1:
                    return input_by_ph, all_ta.nth(0), action
        except Exception as e:
            last_debug = str(e)
...
def run_test():
    ...
    with sync_playwright() as p:
        ...
        for row_index in range(header_row + 1, int(ws.max_row or 0) + 1):
            ...
            try:
                _dismiss_overlays(page)
                # Click Clear button and wait for output box to be empty
                try:
                    clear_btn = page.locator("button", has_text=re.compile(r"Clear", re.IGNORECASE)).first
                    if clear_btn.count() > 0 and clear_btn.is_visible():
                        clear_btn.click()
                        # Wait until output box is actually empty
                        for _ in range(20):
                            page.wait_for_timeout(300)
                            out_val = _read_output(is_chat, output_locator)
                            if not out_val:
                                break
                except Exception:
                    pass
                ...
                if action_locator:
                    try:
                        if action_locator.count() > 0 and action_locator.is_visible():
                            action_locator.click()
                        else:
                            # fallback: find Transliterate button directly
                            btn = page.locator("button", has_text=re.compile(r"Transliterate", re.IGNORECASE)).first
                            if btn.count() > 0 and btn.is_visible():
                                btn.click()
                            else:
                                # last resort: press Enter in input
                ...
                # Wait for output to appear (non-empty) after Transliterate click
                actual_output = ""
                total_wait = max(1000, int(args.wait_ms))
                poll_interval = 500
                elapsed = 0
                while elapsed < total_wait:
                    page.wait_for_timeout(poll_interval)
                    elapsed += poll_interval
                    # Use targeted extraction for chat translator
                    if is_chat:
                        current = _extract_sinhala_only(page)
                    else:
                ...
                # If still empty, try a few more retries
